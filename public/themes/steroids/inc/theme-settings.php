...
if (DISABLE_UI_UPDATES && (get_current_user_id() !== 1 || DISABLE_UI_UPDATES_FOR_FIRST_ADMIN)) {
    add_filter('pre_site_transient_update_core', 'Steroids\hide_updates'); // hide updates for WordPress itself
    add_filter('pre_site_transient_update_plugins', 'Steroids\hide_updates'); // hide updates for all plugins
...
if (DEACTIVATE_GUTENBERG_DEFAULT_CSS) {
...
function upgrade_jquery(&$scripts) {
...
if (OVERRIDE_JQUERY) add_filter('wp_default_scripts', 'Steroids\upgrade_jquery');
...
if (REMOVE_QUERY_STRING) {
    add_filter('style_loader_src', 'Steroids\remove_cssjs_ver', 10, 2);
    add_filter('script_loader_src', 'Steroids\remove_cssjs_ver', 10, 2);
}

// Remove counter-productive auto dns prefetch
if (REMOVE_DNS_PREFETCH) {
...
if (REMOVE_THUMBNAIL_IMG_WIDTH_HEIGHT) {
    add_filter('post_thumbnail_html', 'Steroids\remove_thumbnail_dimensions', 10); // Remove width and height dynamic attributes to thumbnails
    add_filter('image_send_to_editor', 'Steroids\remove_thumbnail_dimensions', 10); // Remove width and height dynamic attributes to post images
}
...
if (REPLACE_SRC_IMG_FILE_BY_LARGE_IMG) {
    ...
    add_filter('wp_generate_attachment_metadata', 'Steroids\replace_uploaded_image');
}
